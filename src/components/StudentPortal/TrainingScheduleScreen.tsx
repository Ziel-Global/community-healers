...
export function TrainingScheduleScreen({
  ...
  const centre = {
    name: centerName || t('Your assigned centre', 'آپ کا مقررہ مرکز'),
    address: centerAddress || null,
    phone: centerPhone || null,
    phoneHref: centerPhone ? centerPhone.replace(/[^\d+]/g, '') : null,
    maps: `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(centerAddress || centerName || '')}`,
  };

  async function copyAddress() {
    if (!centre.address) return;
    try {
  ...
  function addCalendar() {
    const ics = [
      ...
      'SUMMARY:SoftSkills Training Day', 'LOCATION:' + (centre.address || centre.name).replace(/,/g, '\\,'),
          ...
          <div className="training-primary-column">
            ...
            <section className="cp-card centre-card">
              <div className="centre-card-heading">
                ...
                {centerId && <span className="centre-code">{centerId}</span>}
              </div>
