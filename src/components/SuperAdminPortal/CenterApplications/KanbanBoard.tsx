...
interface KanbanBoardProps {
    ...
    inspectorNameById: Map<string, string>;
...
export function KanbanBoard({ applications, cityNameById, inspectorNameById, onRequestAssign, onRequestReject, onRequestApprove }: KanbanBoardProps) {
    ...
    const handleDragEnd = (event: DragEndEvent) => {
        ...
        if (toStatus === "INSPECTION_IN_PROGRESS") {
            if (fromStatus !== "INSPECTION_PENDING") {
                toast({ variant: "destructive", title: "Can't move there", description: "Only new applications can be assigned an inspector." });
                return;
            }
        ...
        if (toStatus === "UNDER_REVIEW") {
            toast({ title: "Moves automatically", description: "This card moves here once the inspector submits their inspection." });
            return;
        }
            ...
            <div className="flex gap-4 overflow-x-auto pb-4 -mx-1 px-1">
                {KANBAN_STATUSES.map((status) => (
                    <KanbanColumn
                        ...
                        inspectorNameById={inspectorNameById}
                    />
                ))}
            </div>
            <DragOverlay>
                {activeApplication && (
                    <div className="rotate-2 scale-105">
                        <ApplicationCard
                            ...
                            inspectorName={activeApplication.inspectorId ? inspectorNameById.get(activeApplication.inspectorId) ?? null : null}
                        />
                    </div>
                )}
            </DragOverlay>
