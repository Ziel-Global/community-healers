...
export function TableView({ applications, cityNameById, inspectorNameById, onRequestAssign, onRequestReject, onRequestApprove }: TableViewProps) {
                ...
                <Table>
                    <TableHeader>
                        <TableRow className="border-b border-border/40 bg-gradient-to-r from-secondary/40 to-secondary/10 hover:bg-gradient-to-r hover:from-secondary/40 hover:to-secondary/10">
                            ...
                            <TableHead className={cn(headCell, "hidden lg:table-cell")}>Inspector</TableHead>
                    ...
                    <TableBody>
                        {pageItems.length === 0 ? (
                            ...
                            pageItems.map((application) => {
                                ...
                                const inspectorName = application.inspectorId ? inspectorNameById.get(application.inspectorId) : null;
                                            ...
                                            {inspectorName ? (
                                                <div className="flex items-center gap-1.5">
                                                    <span className="w-5 h-5 rounded-full gradient-primary text-white text-[9px] font-bold flex items-center justify-center shrink-0">
                                                        {initials(inspectorName.split(" ")[0], inspectorName.split(" ")[1] ?? null, inspectorName)}
                                                    </span>
                                                    <span className="text-xs text-muted-foreground truncate max-w-[120px]">{inspectorName}</span>
                                                </div>
                                            ) : (
