...
import { useCenterApplications, useInspectors } from "@/hooks/queries/useCenterApplicationQueries";
...
export function CenterApplicationsBoard() {
    ...
    const inspectorNameById = useMemo(
        () => new Map(inspectors.map((i) => [i.id, [i.firstName, i.lastName].filter(Boolean).join(" ") || i.email])),
        [inspectors]
    );
